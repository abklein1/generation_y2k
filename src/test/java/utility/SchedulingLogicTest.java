 ...
 * Tests class path determination, vocational decisions, and schedule conflict detection.
 */
...
class SchedulingLogicTest {
    ...
    class ClassPathTests {
        ...
        void testLowIntelligenceFavorsOnLevel() {
            ...
            for (int i = 0; i < trials; i++) {
                ...
                if (path.equals("On-Level")) onLevelCount++;
            }
        ...
        void testIncomeInfluence() {
            ...
            for (int i = 0; i < trials; i++) {
                if (simulateClassPath(70, "high", 50).equals("AP")) highIncomeAP++;
                if (simulateClassPath(70, "low", 50).equals("AP")) lowIncomeAP++;
            }
        ...
        private String simulateClassPath(int intelligence, String income, int determination) {
            ...
            if (intelligence >= 90) {
                apProbability = 50.0; honorsProbability = 35.0; onLevelProbability = 15.0;
            } else if (intelligence >= 70) {
                apProbability = 25.0; honorsProbability = 45.0; onLevelProbability = 30.0;
            } else if (intelligence >= 50) {
                apProbability = 10.0; honorsProbability = 30.0; onLevelProbability = 60.0;
            } else if (intelligence <= 30) {
                apProbability = 2.0; honorsProbability = 8.0; onLevelProbability = 90.0;
            } else {
                apProbability = 5.0; honorsProbability = 20.0; onLevelProbability = 75.0;
            }
            
            switch (income) {
                case "high" -> {
                    apProbability *= 1.3; honorsProbability *= 1.1; onLevelProbability *= 0.8;
                }
                case "low" -> {
                    apProbability *= 0.7; honorsProbability *= 0.9; onLevelProbability *= 1.2;
                }
            }
            ...
            if (random < apProbability) return "AP";
            else if (random < apProbability + honorsProbability) return "Honors";
            else return "On-Level";
        }
    }
    ...
    class PhysicalEducationTests {
        ...
        void testLowDeterminationPreference() {
            // For low determination to apply, strength must be <= 29 (below LOW_STRENGTH_THRESHOLD)
